...
def automatic_guarantee(request, switchgear_id):
    ...
    response['Content-Disposition'] = f'attachment; filename=gwarancja_{switchgear.name}.docx'
    ...
    # DOC PLACE AND DATE
    paragraph_date = document.add_paragraph(f"""Warszawa, dn. {switchgear.actual_shipment}""")
    ...
    # CLIENT BASIC INFO
    d1 = document.add_paragraph(f'Zleceniodawca –  {switchgear.order_ref.ordered_by.name}', style='List Bullet')
    paragraph_format = d1.paragraph_format
    paragraph_format.line_spacing = 1
    d2 = document.add_paragraph(f'Wykonawca – {company.name}', style='List Bullet')
    paragraph_format = d2.paragraph_format
    paragraph_format.line_spacing = 1
    d3 = document.add_paragraph(f'Dotyczy – {switchgear.order_ref.order_name}', style='List Bullet')
    ...
    pattern = '([^\/]+$)'
    production_year = re.search(pattern, switchgear.serial_no)
    p = document.add_paragraph()
    p.add_run(f"""
    ROK PRODUKCJI: {production_year[0]}
    NR FABRYCZNY: {switchgear.serial_no}
    TYP: {switchgear.name}""").font.size = Pt(10)
    ...
    # 18
    document.add_paragraph(
        'Naprawa w miejscu zainstalowana będzie odbywać się przy udziale przedstawiciela Użytkownika.',
        style='List Number'
    )
    ...
    d23 = document.add_paragraph(
        f'Numer telefonu {company.contact_no1},',
        style='List Bullet 2'
    )
    ...
    d25 = document.add_paragraph(
        f'W przypadkach niecierpiących zwłoki – poza godzinami pracy 24/h {company.contact_no2}'
    )
